...
# ===BG Image===================
	...
	login_menu.bg_image=Label(login_menu,image=login_menu.bg).place(anchor='center', relx=0.5, rely=0.5)#x=0,y=0,relwidth=1,relheight=1)
	k_font = tkinter.font.Font(family='Times new roman', size=15, weight=tkinter.font.BOLD)
	

	#id
	id_label=Label(login_menu,relief=GROOVE,text="Your ID:",font=('Cambria',18,'bold'),bg="cyan")
	id_label.place(x=250,y=45)
	id_input_login=Entry(login_menu,width=10,font=("cambria",18,"bold"),bd=5,relief=SUNKEN)
	id_input_login.place(x=370,y=45)

	
	
	# #password
	password_label=Label(login_menu,relief=GROOVE,text="Password:",font=('Cambria',20,'bold'),bg="cyan")
	...
	#login button
	loginbutton1=Button(login_menu,command=login_check,text=" User Login ",bg='red',height=2,width=14,font=k_font)
	loginbutton1.place(x=255,y=600)
	
	# #register button
	registerbutton=Button(login_menu,command=register_in,text=" User Register ",bg='orange',height=2,width=14,font=k_font)
...
def register_in():
	...
	id_label=Label(register_menu,text="Your ID",bd=5,relief=FLAT,font=('arial',20,'bold'))
	# id_label.place(x=370,y=45)
	id_label.grid(row=0,column=1,padx=200,pady=10,sticky="w")


	name_label=Label(register_menu,text="Full Name",bd=5,relief=FLAT,font=('arial',20,'bold'))
	name_label.grid(row=1,column=1,padx=200,pady=10,sticky="w")

	email_label=Label(register_menu,text="Email",bd=5,relief=FLAT,font=('arial',20,'bold'))
...
def admin_in():
		...
		admin_label=Label(admin_menu,bd=5,relief=RIDGE,text="Admin ID",font=('arial',18),bg="light blue")
		admin_password_label=Label(admin_menu,bd=5,relief=RIDGE,text="Password",font=('arial',18),bg="light blue")
		id_admin=Entry(admin_menu,width=10,font=("times new roman",18,"bold"),bd=5,relief=SUNKEN)
		password_admin=Entry(admin_menu,width=10,font=("times new roman",18),bd=5,relief=SUNKEN)
		...
		loginbutton2.place(x=700,y=250)
...
def Admin_Opt():
		...
		opt_menu.bg= PhotoImage(file="images/admin_menu.png")
		opt_menu.bg_image=Label(opt_menu,image=opt_menu.bg).place(anchor='center', relx=0.5, rely=0.5)
		k_font = tkinter.font.Font(family='Calibri', size=50, weight=tkinter.font.BOLD)

		addbutton=Button(opt_menu,command=add_book,bd=5,relief=SUNKEN,text=" Add job posting ",bg='#b365f0',height=1,width=20,font=k_font)
		delbutton=Button(opt_menu,command=del_book,bd=5,relief=SUNKEN,text=" Remove a job posting ",bg='light cyan',height=1,width=20,font=k_font)
...
def add_book():
	...
	add_menu.bg= PhotoImage(file="images/add.png")
	add_menu.bg_image=Label(add_menu,image=add_menu.bg).place(anchor='center', relx=0.5, rely=0.5)

	book_id_label=Label(add_menu,font=('arial',18,'bold'),bg="#A8FBDD",text="Job ID (Should be of atleast length 5):")
	book_label=Label(add_menu,font=('arial',18,'bold'),bg="#A8FBDD",text="Job name:")
	author_label=Label(add_menu,font=('arial',18,'bold'),bg="#A8FBDD",text="Company:")
	book_id=Entry(add_menu)
	book_name=Entry(add_menu)
	author_name=Entry(add_menu)
	addbutton1=Button(add_menu,command=add_check,text=" Add Job ",bg='dark orange',height=1,width=10,font=k_font)
...
def add_check():
	...
	if len(b_name)==0:
		tkinter.messagebox.showinfo("Add","You did not type a Job name O_O")
		add_menu.lift()
		return(add_book)
	...
	if pos != -1:
		tkinter.messagebox.showinfo("Job","Job already present.Please try again")
		add_menu.lift()
		return(add_book)
	...
	f22.close()
	key_sort('Bindex.txt')
	tkinter.messagebox.showinfo("Add","Job added Successfully!")
	add_menu.destroy()
...
def del_book():
	...
	b_label=Label(del_menu,text="Job ID")
	b_label.place(x=10, y=100)
	rb_id=Entry(del_menu)
	delbutton1=Button(del_menu,command=del_check,text="Remove Job",bg='grey',height=1,width=10,font=k_font)
	...
	borrow_label2=Label(del_menu,text="Job")
	borrow_label3=Label(del_menu,text="Company")
...
def Main_Menu():
	...
	base.geometry("1920x720+00+00")
	base.bg= PhotoImage(file="images/user_page.png")
	base.bg_image=Label(base,image=base.bg).place(anchor='center', relx=0.5, rely=0.5)#x=0,y=0,relwidth=1,relheight=1)

	in_font = tkinter.font.Font(family='Calibri', size=25, weight=tkinter.font.BOLD)
	...
	orionLabel=Label(base, text="RECRUITMMENT MANAGEMENT",bg='cyan',font=("calibri", "50","bold"),fg="black")
	...
	#Buttons
	borrow_but=Button(bottomFrame,bg="purple",fg="white",text="Apply for a job",font=in_font,height=3,width=30,command=borrow_in)
	#return_but=Button(bottomFrame,bg="pink",text="Return Outfit",font=in_font,height=3,width=30,command=return_in)
	search_but=Button(bottomFrame,bg="purple",fg="white",text="Search for a job",font=in_font,height=3,width=30,command=search_in)
	feedback_but=Button(bottomFrame,bg="pink",text="Feedback",font=in_font,height=3,width=30,command=feedback_in)
	feedback_but.place(x=700, y=250)
...
def borrow_in():
	...
	borrow_list1.configure(background="aqua")
	borrow_list2.configure(background="aqua")
	borrow_list3.configure(background="aqua")
	borrow_label1=Label(borrow_menu,text="| Please enter the Job ID that you wish to apply for |",font=("Times", "20","bold"),bg="light pink")
	borrow_label=Label(borrow_menu,text="Id")
	borrow_label2=Label(borrow_menu,text="Job")
	borrow_label3=Label(borrow_menu,text="Company")
	borrow_label4=Label(borrow_menu,text="Availablity")

	borrow_entry1=Entry(borrow_menu,width=50)
	borrow_button1=Button(borrow_menu,text="Apply",command=borrow_check,font=("Times new roman","10","bold"),bg="light cyan")
...
def borrow_check():
	...
	for l in f:
		...
		if l[0] ==  id:
			count += 1
	if count >= 2:
		tkinter.messagebox.showinfo("Apply", "Cannot apply for more than 2 jobs at a time")
		borrow_menu.destroy()
		...
		if len(bbook) == 0:
			tkinter.messagebox.showinfo("Appply","You did not type anything O_O")
			borrow_menu.lift()
			return(borrow_in)

		pos = binary_search('Bindex.txt', bbook)
		if pos == -1:
			tkinter.messagebox.showinfo("Apply","The job ID that you had entered is not in our database,sorry,please enter a different job ID")
			borrow_menu.lift()
		else:
			f2 = open('BData.txt', 'r+')
			f2.seek(pos)
			...
			if(w2[3] == 'Y'):
				...
				f2.write(l3)
				f2.close()
				tkinter.messagebox.showinfo("Apply","The job ID you have selected has been successfully applied. Please be ready for interview by:" +'\n'+ str(enddate) )
				...
				f3.close()
				key_sort('Record.txt')
				Done2=tkinter.messagebox.askyesno("Apply","Do you want to apply for another job?")
				if Done2==True:
					borrow_menu.destroy()
					borrow_in()
				else:
				...
				tkinter.messagebox.showinfo("Apply","This job is currently unavailable, please select another job")
				borrow_menu.lift()
...
def search_in():
	...
	search_menu.bg= PhotoImage(file="images/search_menu.png")
	search_menu.bg_image=Label(search_menu,image=search_menu.bg).place(anchor='center', relx=0.5, rely=0.5)

	search_label1=Label(search_menu,text="Search out for the latest job offers provided by our company.\n We provide jobs with healthy environment and future security.",font=("Times", "15","bold","italic"))
	search_label1.place(x=20,y=20,height=70)

	search_entry = Entry(search_menu,width=50,bg="light grey")
	search_entry.place(x=20,y=100,height=40)

	search_button=Button(search_menu,text="Search",height= 1,width= 10,command=search_check,font=("Times new roman","20","bold"),bg="light grey",fg="black")
	search_button.place(anchor='center',x=240,y=200)
...
def search_check():
	...
	if (pos == -1):
		tkinter.messagebox.showinfo("Search","Sorry,this job ID does not exist in our database")
	else:
		search_menu2=Tk()
...
def feedback_in():
	...
	feedback_menu=Toplevel()
	feedback_menu.wm_title("Feedback")
	feedback_menu.geometry("800x600")
	feedback_menu.bg= PhotoImage(file="images/feedback_menu.png")
	feedback_menu.bg_image=Label(feedback_menu,image=feedback_menu.bg).place(anchor='center', relx=0.5, rely=0.5)

	feedback_bar=Entry(feedback_menu,width=130,bg="light grey")
	feedback_label=Label(feedback_menu,text= "Our Happiness lies in your Satisfaction.\n Do share your experience with us.Thank you!",font=("Calibri","30"),bg="light blue")
	...
	feedback_label.place(x=10,y=100)
...
def feedback_read():
	...
	read_feedback_menu.minsize=(2000,2000)
	read_feedback_menu.maxsize=(3000,3000)
